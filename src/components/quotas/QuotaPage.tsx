...
export function QuotaPage() {
  ...
  const families = ['all', ...new Set(data.quotas.map(q => q.family))];
  const types = ['all', ...new Set(data.quotas.map(q => q.type))];
    ...
    <div className="space-y-6">
      ...
      <div className="bg-[hsl(var(--card))] border border-[hsl(var(--border))] rounded-xl overflow-hidden">
        ...
        <div className="px-4 py-3 border-t border-[hsl(var(--border))] text-xs text-[hsl(var(--muted-foreground))]">
          Showing {filtered.length} of {data.quotas.length} quotas
        </div>
      </div>
    </div>
  );
